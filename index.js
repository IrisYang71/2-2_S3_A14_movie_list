...
// 分頁器
function renderPagnitor(amount) {
...
function showMovieModal(id) {
  ...
  axios.get(INDEX_URL + id).then(response => {
    const data = response.data.results //宣告變數放置data的資料
...
// *新增* mode切換
function displayModeSwitch(displayMode) {
  if (dataPanel.dataset.mode === displayMode) return
  dataPanel.dataset.mode = displayMode
}

// *新增* 卡片&清單模式切換的監聽器
changeMode.addEventListener('click', function onModeSwitchClicked (event) {
  if (event.target.matches('#card-btn')) {
    displayModeSwitch('card-mode')
    renderMovieList(getMoviesByPage(currentPage))
  } else if (event.target.matches('#list-btn'))
    displayModeSwitch('list-mode')
    renderMovieList(getMoviesByPage(currentPage))
})
...
paginator.addEventListener('click', function onPaginatorClicked(event) {
  ...
  // 標記目前頁碼
  const activePage = document.querySelector('.page-item')
...
searchForm.addEventListener('submit', function onSearchFormSubmitted(event) {
  ...
  currentPage = 1
  renderPagnitor(filteredMovies.length)
  renderMovieList(getMoviesByPage(currentPage))
})

axios.get(INDEX_URL).then((response) => {
  movies.push(...response.data.results)
  renderPagnitor(movies.length)
  renderMovieList(getMoviesByPage(currentPage))
})